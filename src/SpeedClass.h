...
public:
  ...
  void continueCurrent() { this->tempTimeMs = millis(); }
  ...
  bool getSpeedSensor() { return this->SpeedSensor; }

  void setWheelCircumference(float circumference) {
    this->Memory.wheelCircumference = circumference;
  }

  float getWheelCircumference() { return this->Memory.wheelCircumference; }

  unsigned int getClosestETRTO() {
    int closest = 0;
    for (size_t i = 1; i < NR_ETRTO_WHEELS; i++) {
      float diffLower =
          Wheels[i - 1].circumference - this->Memory.wheelCircumference;
      float diffHigher =
          Wheels[i].circumference - this->Memory.wheelCircumference;
  ...
  void decreaseCircumference(int inc = 1) { this->increaseCircumference(-inc); }
  ...
  int getCadans() { return this->cadans; }

  float getAvgSpeed() { return this->Memory.currentAverageSpeed; }
  ...
  bool isFaster() { return this->speed > this->Memory.currentAverageSpeed; }

  float getCurrentMaxSpeed() { return this->Memory.currentMaxSpeed; }

  float getCurrentDistance() { return this->Memory.currentDistance / 1000; }

  unsigned long getCurrentTime() { return this->Memory.currentTime; }

  unsigned long getTotalTime() {
    return (now() - this->Memory.currentStartTime) * 1000;
  }

  float getDayDistance() { return this->Memory.dayDistance / 1000; }

  float getDayAvgSpeed() { return this->Memory.dayAverageSpeed; }

  float getDayMaxSpeed() { return this->Memory.dayMaxSpeed; }

  unsigned long getDayTime() { return this->Memory.dayTime; }
  ...
  void decreaseTotal(int inc = 1) { this->increaseTotal(-inc); }

  float getPrevDistance() { return this->Memory.prevDistance / 1000; }

  float getPrevAvgSpeed() { return this->Memory.prevAverageSpeed; }

  float getPrevMaxSpeed() { return this->Memory.prevMaxSpeed; }

  unsigned long getprevTime() { return this->Memory.prevTime; }
  ...
  void loop() {
    ...
    if (cadansBuffLength > 0 && totalCadansTime > 0) {
      ...
      this->cadans = int(float(SPEED_CALCULATION_TIME) / avgCadansTime *
                         float(MINUTE / SPEED_CALCULATION_TIME));
    }
    // if (DEBUG)
    // {
    ...
    if (buffLength > 0 && totalSensorTime > 0) {
      ...
      float Speed_meter_sec =
          this->Memory.wheelCircumference / (avgSensorTime / 1000.0);
    ...
    if (this->Memory.currentDistance > 0.1 && this->Memory.currentTime > 1000) {
      this->Memory.currentAverageSpeed = this->Memory.currentDistance /
                                         (this->Memory.currentTime / 1000.0) *
                                         3.6;
      this->Memory.dayAverageSpeed =
          this->Memory.dayDistance / (this->Memory.dayTime / 1000.0) * 3.6;
      // this->Memory.currentAverageSpeed = this->Memory.currentDistance /
    ...
    // Only store ODO if it has changed (per 100m), so EEPROM is only written
    // when needed
  ...
  void _printMemory(const char message[]) {
    ...
    // Serial.print("diff (sec): ");Serial.print(now() -
    // this->Memory.timestamp);

    // Serial.print("Speed: ");Serial.print(this->speed);
    Serial.print("\tCURR:");
    Serial.print("\tDst:");
    Serial.print(this->Memory.currentDistance / 1000);
    Serial.print("\tAvg:");
    Serial.print(this->Memory.currentAverageSpeed);
    Serial.print("\tMax:");
    Serial.print(this->Memory.currentMaxSpeed);
    Serial.print("\tTime:");
    Serial.print(this->Memory.currentTime);
    Serial.print("\tTotal:");
    Serial.print(this->getTotalTime());
    Serial.print("\tstart:");
    Serial.print(this->Memory.currentStartTime);

    Serial.println();
    Serial.print("\tDAY :");
    Serial.print("\tDst:");
    Serial.print(this->Memory.dayDistance / 1000);
    Serial.print("\tAvg:");
    Serial.print(this->Memory.dayAverageSpeed);
    Serial.print("\tMax:");
    Serial.print(this->Memory.dayMaxSpeed);
    Serial.print("\tTime:");
    Serial.print(this->Memory.dayTime);
    Serial.print("\tTotal:");
    Serial.print(this->getDayTotalTime());
    Serial.print("\tstart:");
    Serial.print(this->Memory.dayStartTime);

    Serial.println();
    Serial.print("\tPREV:");
    Serial.print("\tDst:");
    Serial.print(this->Memory.prevDistance / 1000);
    Serial.print("\tAvg:");
    Serial.print(this->Memory.prevAverageSpeed);
    Serial.print("\tMax:");
    Serial.print(this->Memory.prevMaxSpeed);

    // Serial.print("totalDistance: ");Serial.print(this->Memory.totalDistance);
    // Serial.print("tripDistance: ");Serial.print(this->Memory.tripDistance/
    // 1000); Serial.print("wheelCircumference:
  ...
  void speedTrigger() {
    ...
    if (sensorTime > MIN_SENSOR_TIME &&
            sensorTime > this->lastSensorTimeMs / 2 ||
        sensorTime > MIN_SENSOR_TIME * 2) {
